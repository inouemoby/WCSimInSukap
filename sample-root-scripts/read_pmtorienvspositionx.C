...
void read_pmtorienvspositionx(const char* filename = "../rootfile/wcsim_output.root")
{
    char* wcsimdirenv;
	...
	TH2F *angleHist = new TH2F("Angle", "PMT Angle Distribution vs X", 90, 0, 90, 100, -300, 300);

	angleHist->GetXaxis()->SetTitle("Angle [degree]");
	angleHist->GetYaxis()->SetTitle("Photon Position X [cm]");
