...
void read_pmtorienvspositionxy(const char* filename = "../rootfile/wcsim_output.root")
{
  ...
  double lowerThresholdAngle = 0.0;
  double upperThresholdAngle = 30.0;
	...
	char histTitle[50];
	sprintf(histTitle, "PMT Angle (%.1f < Angle < %.1f) Distribution", lowerThresholdAngle, upperThresholdAngle);
	...
	for (int tubeID = 1; tubeID <= numPMTs; ++tubeID)
	{
		...
		for (int x = -300; x <= 300; x = x+6)
		{
			for (int y = -350; y <= 350; y = y+7)
			{
				...
				if (angle > lowerThresholdAngle && angle < upperThresholdAngle) {
					angleHist->Fill(x, y);
				}
			}
		}
