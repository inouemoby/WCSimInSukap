...
void read_pmtid20timingmean(const char* filename = "../rootfile/wcsim_output.root")
{
  ...
  char* wcsimdirenv;
	...
	for (int i = 0; i < 20; ++i) {
	    means[i] = timingHist[i]->GetMean();
	    ids[i] = startID + i;
	    delete timingHist[i]; // delete the histogram after use to save memory
	}

	TCanvas *c2 = new TCanvas("c2", "Mean Timing vs PMT ID", 800, 600);
	TGraph *gr = new TGraph(20, ids, means);
	gr->SetTitle("Mean Timing vs PMT ID");
	gr->GetXaxis()->SetTitle("PMT ID");
	gr->GetYaxis()->SetTitle("Mean Timing [ns]");
